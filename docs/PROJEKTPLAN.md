...
**Untermenü: Mehrsprachigkeit & Lokalisierung (neu, 21.07.)**

Content-Sprache ist je Produkt/Auftrag wählbar — unabhängig vom Marktplatz. Grundprinzip: **lokalisieren, nicht übersetzen** — die Keyword-Analyse muss vom Ziel-Marktplatz kommen, die Kundenstimmen aus der Ziel-Sprache.
...
| Wettbewerber-Vorschlag in 4 Stufen: Kandidaten über Top-Keywords (+ optional Unterkategorie-Bestseller) → Eignungs-Filter (Rating/Rezensionen/BSR als Proxys) → KI-Ähnlichkeits-Gate gegen Produkt-Wahrheit → begründete Vorschlags-Liste zum Bestätigen (D142) | ⬜ | Keyword-Match statt Kategorie-Nachbarschaft; jede Zeile begründet sich selbst · **Datenweg offen:** Suchergebnis-Scrape vs. Cerebro-Weg |
...
| Kunden-Onboarding-Import: ALLE ASINs des Kunden, markenübergreifend (Klarstellung 21.07. — ersetzt „Bestands-Content-Import") | 🔶 teilweise | Zum Start der Zusammenarbeit wird der komplette Kunden-Katalog gescrapt — alle ASINs mit ihrem Live-Content ins Tool. Danach zählt je Produkt nur der Status quo (KEIN Archiv alter Content-Stände — bewusst nicht gebaut). Grundlage: Listing-Import je Produkt ✅; neu: Katalog-Anlage + Massen-Import in einem Zug |
| Soll/Ist-Abgleich (die Mechanik hinter Alerts & Score-Delta — kein eigenes Dashboard) | 🔶 teilweise | Wozu: Nach dem Publish kann Amazon unser Soll STILL verändern (Hauptbild raus, Titel überschrieben, Listing unterdrückt). Der Abgleich vergleicht je Sektion den freigegebenen Stand mit dem frisch gecrawlten Live-Stand — er ist der Detektor, der die Alerts auslöst und erklärt, WAS genau abweicht. Grundlage vorhanden (Soll = Freigaben · Ist = Snapshot; die Analyse kennt beide Seiten heute schon); neu ist nur der Abweichungs-Vergleich. Kontinuierlich erst mit Stufe 2, bis dahin manueller Re-Import |
| Live-Qualitäts-Score (qualitativ — ersetzt das separate Accuracy-Prozent-Dashboard) | 🔶 teilweise | Die HEUTIGE Messung bleibt unverändert (Regel-Messung + Pain-Point-/Kaufauslöser-Abdeckung aus den Analysen — bestätigt 21.07.); sie wird nur zusätzlich dauerhaft auf den LIVE-Stand angewendet. **Dauerlauf heißt: fortlaufend MESSEN, nicht fortlaufend analysieren** — die Datenbasis (Keywords, Pain Points) bleibt eingefroren bis zur bewussten Neu-Analyse; gemessen wird der jeweils frisch gecrawlte Live-Stand gegen diese unveränderte Messlatte (deterministischer Code, keine KI-Kosten). Ein Score-Einbruch entsteht, wenn Amazon den Live-Content ändert (Bild raus, Text überschrieben) — nicht durch neue Analysen. Vollständigkeit steckt automatisch als Fehler-Abzug darin. **Retail-Readiness = Live-Score über Schwelle.** Neu: Crawl-Zyklus + erneute Messung, Soll↔Live-Delta, Cockpit-Kachel |
| Content-Alerts | ⬜ | Auslöser: Soll/Ist-Abweichung oder Live-Score-Einbruch — Hauptbild rausgeflogen, Listing gesperrt/unterdrückt, Text überschrieben. Bestehende Meldungsquellen aufgreifen: Seller-Sentry-/Seller-Central-Benachrichtigungen; mit Stufe 2 prüfen, welche Ereignisse die SP-API-Notifications liefern (Bestand ist zu recherchieren) |
| **Publish zu Amazon per SP-API — das wichtigste CMS-Feature** | 🔒 Stufe 2 | Content direkt aus dem Tool pushen & publishen — ersetzt den Flat-File-Umweg. Gegenstück für Werbung: Kampagnen-Anlage per Ads-API (steht unter Advertising/PPC) |
| Kunden-Feedback am Content-Piece (Kollaborations-Plattform) | ⬜ | Der Kunde bekommt Zugang, sieht seine Content-Pieces und kommentiert DIREKT am Piece — ersetzt Google-Drive-Links mit Kommentarfunktion („nicht sexy"). Braucht Mandanten & Rollen als Unterbau (Stufe-3-Vorstufe) |
