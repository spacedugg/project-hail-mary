...
export async function login(formData: FormData) {
  ...
  if (!email || !password) redirect("/login?fehler=Bitte E-Mail und Passwort eingeben.");

  const db = await getDb();
  const user = await db.query.users.findFirst({ where: eq(schema.users.email, email) });
  if (!user || !verifyPassword(password, user.passwordHash)) {
    redirect("/login?fehler=E-Mail oder Passwort falsch.");
  }
...
export async function register(formData: FormData) {
  ...
  const email = String(formData.get("email") ?? "").trim().toLowerCase();
  const password = String(formData.get("password") ?? "");
  if (!name || !email || !email.includes("@")) redirect("/login?tab=neu&fehler=Bitte Name und gültige E-Mail angeben.");
  if (password.length < 8) redirect("/login?tab=neu&fehler=Passwort braucht mindestens 8 Zeichen.");

  const db = await getDb();
  const existing = await db.query.users.findFirst({ where: eq(schema.users.email, email) });
  if (existing) redirect("/login?fehler=Konto existiert schon — bitte anmelden.");
...
export async function changePassword(formData: FormData) {
  ...
  if (next.length < 8) redirect("/einstellungen?fehler=Neues Passwort braucht mindestens 8 Zeichen.");
